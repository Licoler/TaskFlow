(function () {
  ...
  function toApiPatch(item) {
    var patch = { status: item.done ? 'done' : 'todo' };
  ...
  function enterEditMode(li, item, signal, renderFullCallback) {
    ...
    function saveEdit() {
      ...
      if (newTitle === '') return;
      // Сохраняем в API только title и status, остальное в localStorage
      ...
      request('/api/tasks/' + item.id, { method: 'PATCH', body: JSON.stringify(patchBody) })
        .then(function(updatedTask) {
          ...
          // обновляем модель в глобальном объекте
  ...
  function renderShell(state, signal, actions, renderFullCallback) {
    ...
    // Поле ввода + выбор категории + дедлайн
    ...
    // Обновляем прогресс после рендера
    setTimeout(function() {
      ...
      return function callback() {
        switch(action) {
          case 'ADD':
            ...
            input.disabled = true;
            var category = document.getElementById('new-category') ? document.getElementById('new-category').value : 'personal';
