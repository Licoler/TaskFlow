(function () {
  ...
  submit.type = 'submit';                        // ← явно указываем submit
  submit.textContent = state.authMode === 'login' ? 'Log in' : 'Create account';
  submit.className = 'auth-submit';
  
  var switchBtn = document.createElement('button');
  switchBtn.type = 'button';                     // ← явно НЕ submit!
  ...
  // Всегда берём актуальную модель
  var currentModel = window._taskflow_model || model;
  var total = currentModel.todos.length;
  var completed = currentModel.todos.filter(function(t) { return t.done; }).length;
  var percent = total === 0 ? 0 : (completed / total) * 100;
  
  fill.style.width = percent + '%';
  stats.innerHTML = '<span>' + completed + ' completed</span><span>' + 
    (total - completed) + ' remaining</span>';
  ...
  if (countSpan) {
    var current = window._taskflow_model || model;
    var left = current.todos.filter(function(t) { return !t.done; }).length;
  ...
  // Обновляем и item, и объект в window._taskflow_model
  item.done = newDone;
  var modelItem = window._taskflow_model.todos.find(function(t) { return t.id === item.id; });
  if (modelItem) modelItem.done = newDone;  // ← синхронизируем глобальную модель
  ...
  request('/api/tasks/' + item.id, { 
    method: 'PATCH', 
    body: JSON.stringify({ status: statusFromDone(newDone) }) 
  }).catch(function() {
    // Откат при ошибке
    item.done = !newDone;
  ...
  if (!root) return;
  // Всегда синхронизируем глобальную модель
  window._taskflow_model = state.model;
  empty(root);
  if (!state.token) {
          ...
          case 'ADD':
            ...
            request('/api/tasks', { method: 'POST', body: JSON.stringify({ title: val, status: 'todo' }) })
              .then(function(task) {
                ...
                if (todoList) {
                  var newItem = renderTodoItem(newTask, state.model, signal, renderFull);
