(function () {
  ...
  // Вспомогательная функция для определения просрочки
  ...
  function updateProgressAndConfetti(model) {
    ...
    if (total > 0 && completed === total && typeof canvasConfetti === 'function') {
      canvasConfetti({ particleCount: 150, spread: 70, origin: { y: 0.6 }, startVelocity: 20 });
    }
  }

  // ----- Обновление счётчика оставшихся задач -----
  ...
  // ----- Переключение статуса задачи (оптимистичное обновление) -----
  function toggleTask(item, signal) {
    // Оптимистично меняем DOM
    ...
    if (li) {
      ...
      // Обновляем модель
      item.done = newDone;
      // Обновляем прогресс и счётчик
      var model = { todos: window._taskflow_model.todos };
      updateProgressAndConfetti(model);
      updateCounter(model);
    }
    // Отправляем запрос на сервер (если упадёт, откатим)
    patchTask(item.id, { status: statusFromDone(!item.done) }).catch(function() {
      // Откат
      if (li) {
        var checkBack = li.querySelector('.todo-checkbox');
        if (checkBack) checkBack.checked = item.done;
        if (item.done) li.classList.add('completed');
        else li.classList.remove('completed');
      }
      item.done = !item.done;
      updateProgressAndConfetti({ todos: window._taskflow_model.todos });
      updateCounter({ todos: window._taskflow_model.todos });
    });
  }
  ...
  // ----- Рендер главного интерфейса (с фильтрами по категориям) -----
  function renderShell(state, signal, actions) {
    ...
    var total = state.model.todos.length;
    var completed = state.model.todos.filter(function(t) { return t.done; }).length;
    var percent = total === 0 ? 0 : (completed / total) * 100;
    progressFill.style.width = percent + '%';
    progressStats.innerHTML = '<span>' + completed + ' completed</span><span>' + (total - completed) + ' remaining</span>';
    ...
    // Список задач с фильтрацией по категории и статусу
  ...
  // Рендер одной задачи с возможностью редактирования (двойной клик)
  function renderTodoItem(item, model, signal) {
    ...
    var categoryLabel = { personal: '👤', work: '💼', study: '📚' }[item.category] || '📌';
    ...
    li.addEventListener('dblclick', function() {
      enterEditMode(li, item, signal);
    });

    return li;
  }

  function enterEditMode(li, item, signal) {
    // Скрываем обычное содержимое
    ...
    ['personal', 'work', 'study'].forEach(function(cat) {
    ...
    function saveEdit() {
      ...
      var newDeadlineStr = newDeadline ? new Date(newDeadline).toISOString() : '';
      var patchBody = { title: newTitle };
      if (newDeadlineStr !== item.deadline) patchBody.deadline = newDeadlineStr;
      if (newCategory !== item.category) patchBody.category = newCategory;
      request('/api/tasks/' + item.id, { method: 'PATCH', body: JSON.stringify(patchBody) })
        .then(function(updatedTask) {
          item.title = updatedTask.title;
          item.deadline = updatedTask.deadline || '';
          item.category = updatedTask.category || 'personal';
          var newLi = renderTodoItem(item, window._taskflow_model, signal);
          li.parentNode.replaceChild(newLi, li);
          window._taskflow_model.todos = window._taskflow_model.todos.map(function(t) {
            return t.id === item.id ? item : t;
          });
  ...
  // ----- Глобальная функция рендера (пересоздаёт всё, но используется только при добавлении/удалении/очистке) -----
  var renderFull = function(model) {
    if (!window._taskflow_root) return;
    empty(window._taskflow_root);
    window._taskflow_root.appendChild(renderShell(window._taskflow_state, window._taskflow_signal, window._taskflow_actions));
    updateProgressAndConfetti(model);
    updateCounter(model);
  };
  ...
  function mountTaskFlow(initialModel, rootElementId) {
    ...
    window._taskflow_actions = actions;
      ...
      return function callback() {
        switch(action) {
          case 'ADD':
            ...
            input.disabled = true;
            request('/api/tasks', { method: 'POST', body: JSON.stringify({ title: val, status: 'todo', deadline: '', category: 'personal' }) }).then(function(task) {
            ...
            break;
          case 'DELETE':
            request('/api/tasks/' + data, { method: 'DELETE' }).then(function() {
              deleteTaskWithAnimation(data, state.model, renderFull);
            }).catch(setError);
            break;
          case 'CLEAR_COMPLETED':
            ...
            break;
          default:
            // для TOGGLE_ALL и других - можно оставить перерендер
    ...
    window._taskflow_signal = signal;
